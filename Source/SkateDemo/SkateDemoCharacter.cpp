...
void ASkateDemoCharacter::Move(const FInputActionValue& Value)
{
	...
	const FVector SkateForwardVector = SkateBoardSkeletalMesh->GetForwardVector();
	const FVector SakateRightVector = SkateBoardSkeletalMesh->GetRightVector();

	// add movement 
	AddMovementInput(SkateForwardVector, MovementVector.Y);
	AddMovementInput(SakateRightVector, MovementVector.X);
}
